...
// Debug mode: uncomment to see detailed logs
...
// Convert int8 features to normalized float (0.0 - 1.0)
void ConvertInt8ToFloat(const int8_t* int8_data, float* float_data, int size, 
                        float scale, int zero_point) {
  // First pass: dequantize and find actual min/max
  float temp[size];
  float min_dequant = 1e9, max_dequant = -1e9;
  
  for (int i = 0; i < size; i++) {
    // Dequantize: real_value = (quantized_value - zero_point) * scale
    temp[i] = (int8_data[i] - zero_point) * scale;
    if (temp[i] < min_dequant) min_dequant = temp[i];
    if (temp[i] > max_dequant) max_dequant = temp[i];
  }
  
  // Second pass: normalize to [0, 1] using Min-Max normalization
  float range = max_dequant - min_dequant;
  if (range < 1e-6) {
    ...
    for (int i = 0; i < size; i++) {
      float_data[i] = (temp[i] - min_dequant) / range;
      // Clip to ensure valid range
      if (float_data[i] < 0.0f) float_data[i] = 0.0f;
      if (float_data[i] > 1.0f) float_data[i] = 1.0f;
    }
  }
}
...
void setup() {
  ...
  // We'll extract features from the model output
  // Note: In this setup, we use the CNN output directly as features
  TfLiteTensor* model_output = interpreter->output(0);
  if ((model_output->dims->size != 2) || (model_output->dims->data[0] != 1)) {
    MicroPrintf("Unexpected output tensor structure");
    // Continue anyway - we'll handle it in the loop
  }
  
  // Initialize classification head
  setup_head(NN_DEF, sizeof(NN_DEF)/sizeof(NN_DEF[0]), SAVED_WEIGHTS);
  MicroPrintf("Classification head initialized!");
}
...
void loop() {
  ...
  if (done_just_triggered) {
    ...
    // Step 1: Run CNN backbone to extract features
    TfLiteTensor* model_input = interpreter->input(0);
    ...
    // Step 2: Extract 64-dimensional features from CNN output
    TfLiteTensor* feature_tensor = interpreter->output(0);
    
    // Validate feature dimensions
    int actual_feature_size = 1;
    for (int j = 0; j < feature_tensor->dims->size; j++) {
      actual_feature_size *= feature_tensor->dims->data[j];
    }
    
    if (actual_feature_size != feature_dim) {
      MicroPrintf("ERROR: CNN outputs %d dims, but classifier expects %d", 
                  actual_feature_size, feature_dim);
      return;  // Skip this inference
    }

    // Step 3: Convert int8 features to normalized float [0.0, 1.0]
    ConvertInt8ToFloat(feature_tensor->data.int8, 
                       extracted_features, 
                       feature_dim,
                       feature_tensor->params.scale,
                       feature_tensor->params.zero_point);

    // Step 4: Run classification head
    float head_output[label_count];
    run_inference(extracted_features, head_output);

    // Step 5: Find max probability and class
    float max_score = head_output[0];
    int max_index = 0;
    for (int i = 1; i < label_count; ++i) {
      if (head_output[i] > max_score) {
        max_score = head_output[i];
        max_index = i;
      }
    }
    
    float max_score_int;
    float max_score_frac = modf(max_score * 100, &max_score_int);
