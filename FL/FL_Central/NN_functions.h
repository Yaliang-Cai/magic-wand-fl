...
// 替换或添加到 NN_functions.h
void printDetailedTest() {
  Serial.println("\n--- DETAILED PREDICTIONS (Test Data) ---");
  int correct = 0;
  
  // 遍历测试集 (data_A.h 里的 test_data 包含 0,1,2)
  for (int i = 0; i < test_data_cnt; i++) {
    ...
    for (int j = 1; j < OUT_VEC_SIZE; j++) {
      if (y[predicted] < y[j]) predicted = j;
    }
    ...
    Serial.print(" -> Pred: "); Serial.print(predicted);
    
    // 打印置信度 (看看它对 Class 2 的概率是不是在上升)
    Serial.print(" [Prob: 0="); Serial.print(y[0], 2);
    Serial.print(", 1="); Serial.print(y[1], 2);
    Serial.print(", 2="); Serial.print(y[2], 2);
    Serial.print("]");
  ...
  float acc = (float)correct / test_data_cnt;
  Serial.print("Final Detailed Accuracy: "); Serial.println(acc);
}
